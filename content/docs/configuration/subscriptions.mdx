...
2.  **Endpoint URL:** For local development, use a tool like the Stripe CLI to forward events to your local application. For production, enter your live domain's webhook URL (e.g., `https://getcracked.lol/api/webhooks/stripe`).
...
2.  Add a new endpoint that points to your **production domain** (e.g., `https://getcracked.lol/api/webhooks/stripe`).
...
<Callout type="info">
  **Webhook URL Format**: `https://getcracked.lol/api/webhooks/stripe`

  Remember to replace `getcracked.lol` with your actual production domain.
</Callout>
